...
EOF
          ...
          opts.on('-c', '--create[=FREQ,SPAN]', Array,
                  "For each dataset given, create snapshot named per
                       specified FREQUENCY and LIFESPAN. If option arguments
                       are not specified, they default to: #{Snap::DEFAULTS.values.join(',')}.") do |list|
            @option[:create] =
          ...
          opts.on('-e', '--expire',
                  "Destroy expired snapshots matching regular expressions
                       REGEX ... . If no regular expression is specified,
                       then all expired snapshots are destroyed.") do
            @option[:expire] = true
          end

          opts.on("-l", "--list",
                  "List snapshots matching regular expressions REGEX ...
                       If no regular expression is specified, then all
                       snapshots are listed.") do
            @option[:list] = true
          end

          opts.on("-n", "--no-execute",
                  "When combined with one of the flags -c, -d or -e,
                       display commands that would be executed, but
                       don't actually execute them.") do
            @option[:no_exec] = true
            @option[:verbose] = true
          end
